...
template <class TV> struct basic_xbox
{
	...
	TV corner( size_t index ) const
	{
		...
		{
			case 0:
				return cross_over<0>( min, max );
			case 1:
				return cross_over<1>( min, max );
			case 2:
				return cross_over<2>( min, max );
			case 3:
				return cross_over<3>( min, max );
			case 4:
				return cross_over<4>( min, max );
			case 5:
				return cross_over<5>( min, max );
			case 6:
				return cross_over<6>( min, max );
			case 7:
				return cross_over<7>( min, max );
		}
...
template <typename T> struct xmat4 : xmat_data<T, 4>
{
	...
	{
		return xmat4( m[0] * n[ 0] + m[4] * n[ 1] + m[8] * n[ 2] + m[12] * n[ 3], m[1] * n[ 0] + m[5] * n[ 1] + m[9] * n[ 2] + m[13] * n[ 3],
		              ...
		              m[0] * n[ 8] + m[4] * n[ 9] + m[8] * n[10] + m[12] * n[11], m[1] * n[ 8] + m[5] * n[ 9] + m[9] * n[10] + m[13] * n[11],
		              m[2] * n[ 8] + m[6] * n[ 9] + m[10] * n[10] + m[14] * n[11],  m[3] * n[ 8] + m[7] * n[ 9] + m[11] * n[10] + m[15] * n[11],
		              m[0] * n[12] + m[4] * n[13] + m[8] * n[14] + m[12] * n[15], m[1] * n[12] + m[5] * n[13] + m[9] * n[14] + m[13] * n[15],
		              m[2] * n[12] + m[6] * n[13] + m[10] * n[14] + m[14] * n[15],  m[3] * n[12] + m[7] * n[13] + m[11] * n[14] + m[15] * n[15] );
	}
