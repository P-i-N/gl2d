...
namespace fontconv
{
	...
	public class Program
	{
		public static int Main(string[] args)
		{
			...
			p.Run(new string[] { "..\\..\\data\\fonts\\Monospace.fnt" });
		...
		public int Run(string[] args)
		{
			try
			{
				...
				var rleBase64 = CompressBase64(base64);
				...
				tw.WriteLine("static const size_t " + fileName.ToLower() + "_base64_length = " + base64.Length.ToString() + ";");
				tw.WriteLine("static const char *" + fileName.ToLower() + "_base64 =");

				int step = 120;
				for (int i = 0; i < rleBase64.Length; i += step)
				{
					bool isLastLine = (i + step) >= rleBase64.Length;
					string line = isLastLine ? rleBase64.Substring(i) : rleBase64.Substring(i, step);

					tw.WriteLine("\"" + line + (isLastLine ? "\";" : "\""));
				}
